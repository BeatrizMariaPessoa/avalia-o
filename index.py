...
from templates.editar_perfil import EditarPerfil
from templates.agendarhorarioUI import AgendarHorarioUI
from templates.vizualizarUI import VizualizarUI
...
class IndexUI:
  ...
  def menu_admin():
    op = st.sidebar.selectbox("Menu", ["Manter Agenda", "Manter Clientes", "Manter Serviços", "Abrir Agenda do Dia", "Reajustar Preço", "Editar Perfil", "Confirmar"])
  ...
  def menu_cliente():
    op = st.sidebar.selectbox("Menu", ["Agenda de Hoje", "Editar Perfil", "Agendar Horário", "Vizualização de Horários"])
    if op == "Agenda de Hoje": AgendaHojeUI.main()
    if op == "Editar Perfil": EditarPerfil.main()
    if op == "Agendar Horário": AgendarHorarioUI.main()
    if op == "Vizualização de Horários": VizualizarUI.main()
