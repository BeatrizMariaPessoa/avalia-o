...
class View:
  ...
  def agenda_listar_seguintes():
    r = []
    hoje = datetime.datetime.today()
    uma_semana = hoje.date() + datetime.timedelta(days = 7)
    for horario in View.agenda_listar():
      if horario.get_confirmado() == False and horario.get_data().date() > hoje.date() and horario.get_data().date() <= uma_semana:
        r.append(horario)
    return r
